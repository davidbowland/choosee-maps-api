...
import { choiceId } from '../__mocks__'
...
describe('post-start-prune', () => {
  ...
  beforeAll(() => {
    ...
    mocked(dynamodb).scanExpiredIds.mockResolvedValue([choiceId])
  })
  ...
  describe('postStartPruneHandler', () => {
    ...
    test('expect choiceId passed to deleteDataById', async () => {
      await postStartPruneHandler(event)
      expect(mocked(dynamodb).deleteDataById).toHaveBeenCalledWith(choiceId)
    })
